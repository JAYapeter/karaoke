...
- **Host IP changes during session.** `serverHost` is captured once at boot and does not refresh. If the host switches networks (Wi-Fi reconnect, VPN toggle, DHCP renewal, Wi-Fi ↔ wired), the QR keeps encoding the boot-time IP and new joiners scanning will hit a dead address. Workaround: restart the server, or set `KARAOKE_LAN_HOST` to a stable mDNS hostname (e.g., `shimokita.local:3000`) that survives IP changes.
